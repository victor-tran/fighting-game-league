class RelationshipsController < ApplicationController
  ...
  def create
    ...
    n = @user.notifications.create!(sendable_id: current_user.id,
                                    ...
                                    content: Notification.followed_user(current_user),
                                    read: false)
    # Send a push notification via Pusher API to @user.
    if current_user.avatar_file_name == nil
      Pusher['private-user-'+@user.id.to_s].trigger('user_notification',
                                                     { follower_id: current_user.id,
                                                       unread_count: @user.notifications.unread.count,
                                                       notification_content: Notification.followed_user(current_user),
                                                       no_avatar: true,
  ...
  def destroy
    ...
    n = @user.notifications.find_by(sendable_id: current_user.id,
                                    ...
                                    targetable_id: @user.id,
                                    targetable_type: 'User',
                                    content: Notification.followed_user(current_user)).destroy
